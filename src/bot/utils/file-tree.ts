...
export function pathToDisplayPath(absolutePath: string): string {
  ...
  if (absolutePath.startsWith(`${homeDirectory}${path.sep}`)) {
...
export async function scanDirectory(
  ...
  try {
    ...
    subdirectories.sort((left, right) =>
      left.name.localeCompare(right.name, undefined, { sensitivity: "base" }),
    );

    const totalPages = Math.max(1, Math.ceil(subdirectories.length / MAX_ENTRIES_PER_PAGE));
    const safePage = Math.max(0, Math.min(page, totalPages - 1));
    const startIndex = safePage * MAX_ENTRIES_PER_PAGE;
    const pageEntries = subdirectories.slice(startIndex, startIndex + MAX_ENTRIES_PER_PAGE);
    const parentPath = path.dirname(dirPath);
    const hasParent = dirPath !== path.parse(dirPath).root;

    return {
      entries: pageEntries,
      totalCount: subdirectories.length,
      page: safePage,
      currentPath: dirPath,
